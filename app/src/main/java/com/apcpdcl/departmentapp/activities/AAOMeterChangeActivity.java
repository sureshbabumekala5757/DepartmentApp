...
public class AAOMeterChangeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.aao_meter_change_activity);
    ...
    void reject() {
        ...
        if (Utility.isValueNullOrEmpty(mRemarks)){
            Utility.showCustomOKOnlyDialog(this,"Please Select Reason for Rejection.");
        }else if (mRemarks.equalsIgnoreCase("Others") &&
                Utility.isValueNullOrEmpty(et_remarks.getText().toString())) {
            Utility.showCustomOKOnlyDialog(this,"Please Enter Reason for Rejection.");
        } else {
            if (Utility.isNetworkAvailable(this)) {
                changeStatusMeterDetails("R");
            } else {
                Utility.showCustomOKOnlyDialog(this, Utility.getResourcesString(this, R.string.no_internet));
            }
        }
...
*/
    ...
    private void getMeterDetails() {
        ...
        client.post(Constants.METER_CHANGE_URL + Constants.MODIFY_MTR_DETAILS, params, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(String response) {
                ...
                try {
                    ...
                    if (jsonObject.has("MSG")) {
                        Utility.showCustomOKOnlyDialog(AAOMeterChangeActivity.this, "Not a valid Service Number");
                    } else {
            ...
            public void onFailure(int statusCode, Throwable error, String content) {
                ...
                switch (statusCode) {
                    case 404:
                        Utility.showCustomOKOnlyDialog(AAOMeterChangeActivity.this, "Unable to Connect Server");
                        break;
                    case 500:
                        Utility.showCustomOKOnlyDialog(AAOMeterChangeActivity.this, "Something went wrong at server end");
                        break;
                    default:
                        Utility.showCustomOKOnlyDialog(AAOMeterChangeActivity.this, "Check Your Internet Connection and Try Again");
    ...
    private void setData() {
        ...
        et_changed_by.setText(mMeterChangeModel.getMTRCHANGEDBY());
    ...
    /* *Set Meter change reason Spinner Data**/
    private void setRemarksSpinnerData() {
        final ArrayList<String> list = new ArrayList<>();
        list.add("--Select--");
        list.add("Meter change slip not yet received.");
        list.add("Meter change particulars not tallied.");
        list.add("Final reading of the removed meter is very abnormal.");
        list.add("Old Meter change particulars tallied.");
        list.add("New Meter change particulars tallied.");
        list.add("Others");

        SpinnerAdapter newlineAdapter = new SpinnerAdapter(this,
                android.R.layout.simple_spinner_item, list);
        spn_remarks.setAdapter(newlineAdapter);
        spn_remarks.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (position == list.indexOf("Others")) {
                    ...
                    mRemarks = parent.getItemAtPosition(position).toString();
                } else if (position != 0) {
                    mRemarks = parent.getItemAtPosition(position).toString();
    ...
    /*Accept or Reject  METER CHANGE DETAILS*/
    private void changeStatusMeterDetails(String flag) {
        RequestParams params = new RequestParams();
        ...
        if (mRemarks.equalsIgnoreCase("Others")) {
        ...
        AsyncHttpClient client = new AsyncHttpClient();
        Utility.showLog("URL", Constants.URL_ERO + "EROservices/update");
        Utility.showLog(Constants.USCNO, mServiceNum);
        Utility.showLog(Constants.MTRCHGDT, mMeterChgDate);
        Utility.showLog(Constants.NEWMTRNO, mNewMtrNum);
        // client.addHeader("SCNO", scno);
        client.post(Constants.URL_ERO + "EROservices/update", params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(String response) {
                ...
                try {
                    ...
                    if (jsonObject.has("Status")) {
                        if (jsonObject.optString("Status").equalsIgnoreCase("TRUE")) {
                            AAOMeterChangeListActivity.getInstance().updateList(mServiceNum);
                            Utility.showToastMessage(AAOMeterChangeActivity.this, jsonObject.optString("Message"));
                            finish();
                        } else if (jsonObject.has("Message")) {
                            Utility.showCustomOKOnlyDialog(AAOMeterChangeActivity.this,
